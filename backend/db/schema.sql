...
-- staff pools) -- see that migration's own docstring for the full reasoning.
...
-- CANCELLED) + price-at-booking snapshots + an optional order/prescription
...
-- Dine Connect fork: patient_visit_notes/patient_documents (clinical notes,
-- prescriptions/lab reports) dropped entirely -- no dining equivalent
-- (ARCHITECTURE_REFERENCE_FOR_FORKING.md Section 5). Visit HISTORY itself
-- (get_patient_visit_history()) is unaffected -- it reads `appointments`
-- directly, never needed either of these tables.
