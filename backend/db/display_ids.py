...
# Reservation reference id (generate_reference_id() below) -- RSV-<DDMMYY>-<NNN>,
# e.g. RSV-130826-001, shown directly to guests/staff on confirmations. Was
# "APT-" (appointment) before the restaurant fork; reservations made before
# the rename keep their APT- ids (the counter row is keyed by prefix, so the
# two sequences never collide). Deliberately NOT moved onto code_sequences
# (see module docstring).
APPOINTMENT_REFERENCE_ID_PREFIX = "RSV"

# Food order reference id (food ordering plan, Sub-stage 1) -- ORD-<DDMMYY>-
# <NNN>, same daily/per-hospital shape as APPOINTMENT_REFERENCE_ID_PREFIX
# above, generated by the same _generate_reference_id() (now parameterized
# by prefix) -- but its own independent counter row (reference_id_counters
# widened to (hospital_id, day, prefix) by migration 0031), confirmed with
# the user to never collide with or be confusable with a reservation's own
# RSV- reference id.
ORDER_REFERENCE_ID_PREFIX = "ORD"
...
def _next_daily_reference_sequence(conn, hospital_id: int, day: str, prefix: str = APPOINTMENT_REFERENCE_ID_PREFIX) -> int:
    """Atomic per-(hospital, day, prefix) increment -- INSERT...ON CONFLICT DO
    UPDATE is a single statement, so this is race-safe under real concurrent
    bookings (unlike a read-then-increment-then-write pair). Deliberately its
    own reference_id_counters table, not code_sequences -- see module
    docstring for why APT stays out of the shared/yearly-reset scheme.

    `prefix` defaults to APT for every pre-existing call site; migration 0031
    widened the table's PK to (hospital_id, day, prefix) specifically so a
    second prefix (ORD, food ordering) can share this same mechanism with its
    own fully independent counter, never colliding with APT's."""
    row = conn.execute(
        "INSERT INTO reference_id_counters (hospital_id, day, prefix, counter) VALUES (?, ?, ?, 1) "
        "ON CONFLICT (hospital_id, day, prefix) DO UPDATE SET counter = reference_id_counters.counter + 1 "
        "RETURNING counter",
        (hospital_id, day, prefix),
    ).fetchone()
    return row["counter"]


def _generate_reference_id(
    conn, hospital_id: int, now: datetime | None = None, prefix: str = APPOINTMENT_REFERENCE_ID_PREFIX,
) -> str:
    ...
    `prefix` defaults to APT; food ordering (Sub-stage 2) passes
    ORDER_REFERENCE_ID_PREFIX ("ORD") to get its own independent sequence via
    the same (hospital_id, day, prefix)-keyed counter row."""
    now = now or datetime.now()
    day_key = now.strftime("%Y-%m-%d")
    seq = _next_daily_reference_sequence(conn, hospital_id, day_key, prefix)
