    ...
    try:
        ...
        cur = conn.execute(
            ...
            (hospital_id, phone, department_id, None, scheduled_at_iso, 0, SOURCE_WHATSAPP,
             ...
             "procedure", procedure_id, "CONFIRMED", procedure["estimated_price_min"], procedure["estimated_price_max"],
...
def create_procedure_request(
    ...
    The extra nullable column procedure_status carries the "is this really
    ...
    cur = conn.execute(
        ...
        (hospital_id, phone, department_id, None, now.isoformat(), 0, SOURCE_WHATSAPP,
         _generate_reference_id(conn, hospital_id), patient["id"], patient["name"], phone, patient["age"],
         "procedure", procedure_id, "REQUESTED", procedure["estimated_price_min"], procedure["estimated_price_max"],
         procedure_order_reference),
    )
    new_id_row = cur.fetchone()
    assert new_id_row is not None
...
def set_procedure_status(hospital_id: int, appointment_id: int, procedure_status: str) -> Appointment | None:
    ...
    portal/routes/bookings.py) -- a plain single-column UPDATE. Never used
    for CONFIRMED (that's confirm_procedure_appointment()'s own job,
    since it also reserves resources and moves scheduled_at atomically)."""
    session = get_session()
